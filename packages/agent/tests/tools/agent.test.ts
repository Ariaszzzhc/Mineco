...
import { mockProvider, textChunk, finishChunk } from "../helper/mock-provider.js";
...
function makeSessionStore() {
  ...
  return {
    create: async () => ({ id: "s1", title: "t", workspaceId: "w1", messages: [], createdAt: Date.now(), updatedAt: Date.now() }),
    get: async () => undefined,
    list: async () => [],
    ...
    createRun: async (run: { id: string }) => {
      runs.push({ id: run.id, status: "running", summary: null, completedAt: null });
    },
    updateRun: async (
      runId: string,
      updates: { status?: "running" | "completed" | "error"; summary?: string | null; completedAt?: number | null },
    ) => {
...
describe("Agent Tool", () => {
  it("returns error for unknown agent type", async () => {
    ...
    const registry = { get: () => provider, acquireRateLimit: async () => {} } as unknown as ProviderRegistry;
    const definitions = makeDefinitions({ type: "explore", toolNames: ["read_file"], maxSteps: 5 });
    const sessionStore = makeSessionStore();

    const tool = createAgentTool({ providerRegistry: registry, definitions, sessionStore });
