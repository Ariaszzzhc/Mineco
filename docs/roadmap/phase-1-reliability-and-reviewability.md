...
- agent 调用 `plan.submit` runtime tool 提交该 plan。
- ToolRuntime 写入 `kind="plan"` 的 artifact，并保存 `SummaryItem(kind="plan")`；大计划内容写 artifact，summary 中保留 artifact ref。
- ToolRuntime 为这次 `plan.submit` tool call 发出 `approval.requested`。
- 用户通过 Runtime SDK `decideApproval(request.id, decision)` 批准后，runtime 发出 `approval.decided`，恢复进入 Plan mode 前的 permission mode，并以该 plan 作为执行上下文继续。
...
- `plan.submit`：Plan mode 专用 runtime tool，输入计划内容，输出 plan artifact ref，并强制触发 approval。
...
- `artifacts`：补齐 `run_id`、kind、mime、size、sha256、model-visible summary；支持 `terminal_log`、`diff`、`report`、`plan`，并遵守协议文档中的 `ArtifactKind` union。
...
- 实现 read-only Plan mode、`plan.submit`、plan artifact 和既有 tool approval。
- 执行后输出轻量 plan 对照结果。

验收：Plan mode 下写入和副作用工具被拒绝；`plan.submit` approval 后恢复原权限模式执行。
...
- Plan mode 只读工具可用，写入工具被拒绝，`plan.submit` approval 后可执行。
...
7. 启动 Plan mode，确认只读计划、`plan.submit` approval 和批准后执行路径。
...
- Plan mode 是只读 permission mode，能通过 `plan.submit` 生成 plan artifact，并通过该 tool approval 才进入执行。
