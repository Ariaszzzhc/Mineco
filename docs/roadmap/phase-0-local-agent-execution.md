...
- 技术栈采用 Node.js-first：TypeScript、Vitest、Zod、Kysely、Effect、`node:sqlite`。
...
State Store
  .electrolyte/runtime.db SQLite metadata and append-only logical logs,
...
- Toolchain：`tsc --noEmit`、`vitest run`、formatter/linter 后续按实现需要选择。
...
electrolyte> 找出测试失败原因并修复
assistant: ...
tool.started file.search {...}
tool.finished file.search ok
approval shell.run risk=medium command="vitest run" [y/n] y
...
- 大输出写入 `.electrolyte/artifacts/`，SQLite 只保存 metadata 和摘要。
...
所有自动测试使用 Vitest，默认命令为 `vitest run`。
...
- REPL 展示工具调用和测试输出。
- 最终说明改了什么、跑了什么验证。
- `.electrolyte/runtime.db` 中能看到 session 历史，`.electrolyte/artifacts/` 中保存大输出 artifact。
