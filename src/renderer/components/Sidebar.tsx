                ...
                historySessions.map((session) => (
                  ...
                  >
                    {session.id === currentSessionId ? (
                      <CircleDot size={10} className="text-primary shrink-0" strokeWidth={2} />
                    ) : (
                      <Code size={14} className="opacity-60 shrink-0" strokeWidth={1.5} />
                    )}
                    <span className={`truncate flex-1 ${session.id === currentSessionId ? 'text-text-primary font-medium' : ''}`}>
